...
SQL_void("create table if not exists articles ( id integer not null auto_increment, title text, called text, content text, credits text, another text, img_src text, createdAt datetime, updatedAt datetime, primary key (id))")
...
app.get('/getAtualizacoes', async function(req, res){     
    SQL_JSON("select * from articles order by createdAt desc limit 5", res)   
});

app.post('/insertPost', async function(req, res){   
    console.log(req.body)
    var dados = req.body.FEED
    console.log(dados)
    var titulo, chamada, conteudo, img, creditos, categoria
    for(const k in dados){
        if(dados[k].tag == "TÍTULO"){titulo = dados[k].html.replace(/'/g,'"')}
        if(dados[k].tag == "CHAMADA"){chamada = dados[k].html.replace(/'/g,'"')}
        if(dados[k].tag == "CONTEUDO"){conteudo = dados[k].html.replace(/'/g,'"')}
        if(dados[k].tag == "CREDITOS"){creditos = dados[k].html.replace(/'/g,'"')}
        if(dados[k].tag == "IMAGEM"){img = dados[k].html.replace(/'/g,'"')}
        if(dados[k].tag == "CATEGORIA"){categoria = dados[k].html.replace(/'/g,'"')}
    }
    console.log("insert into articles values ( null, '"+titulo+"','"+chamada+"','"+conteudo+"','"+creditos+"','"+categoria+"',0,'"+img+"',now(),now())")
    SQL_void("insert into articles values ( null, '"+titulo+"','"+chamada+"','"+conteudo+"','"+creditos+"','"+categoria+"',0,'"+img+"',now(),now())")
    res.send({"status":"OK"})
...
app.post('/insertComment', async function(req, res){ 
    console.log("req.body")
    console.log(req.body)
     console.log(req.body.dados)
     console.log("req.body MEIO")
     
    SQL_JSON("insert into comentarios (id, idComentario, nome, email, comentario, createdAt) values (null, "+req.body.dados.id+", '"+req.body.dados.nome+"', '"+req.body.dados.email+"', '"+req.body.dados.comentario+"', now())", res)   
        console.log("req.body FIM")
});

app.post('/insertEmail', async function(req, res){ 
    console.log(req.body.IMAIUL)
    SQL_JSON("insert into newsletter  values (null, '"+req.body.IMAIUL[0].email+"',now())",res)   
  //  

   
      var  html = ''
      enviaEmail("gamora@printmer.com.br", req.body.IMAIUL[0].email, "Newsletter Tech BLog da Gamora", html)

      enviaEmail("gamora@printmer.com.br", 'rounantj@hotmail.com', "Nova assinatura no seu site", req.body.IMAIUL[0].email+" assinou")


  
});

app.post('/insertContato', async function(req, res){ 
    console.log(req.body.IMAIUL)

   
      var  html = 'Nome: '+req.body.IMAIUL[0].nome+'<br>'+
                  'E-mail: '+req.body.IMAIUL[0].email+'<br>'+
                  'Telefone: '+req.body.IMAIUL[0].telefone+'<br>'+
                  'Assunto: '+req.body.IMAIUL[0].assunto+'<br>'+
                  'Mensagem: "'+req.body.IMAIUL[0].mensagem.replace(/'/g,'')+'"<br>';

      enviaEmail("gamora@printmer.com.br", req.body.IMAIUL[0].email, "Contato com Tech BLog da Gamora", "<h1>Obrigado pelo contato, retornaremos em breve!</h1>")

      enviaEmail("gamora@printmer.com.br", 'rounantj@hotmail.com', "Novo contato no seu site", html)

      res.send("Enviado aos solicitados...")
...
app.get('/updateViews/:id', async function(req, res){     
    SQL_JSON("update articles set views = views + 1 where id = "+req.params.id, res)   
});

app.get('/getArticle/:title', async function(req, res){  
    console.log("select * from articles where title like '%"+req.params.title+"%'")
    SQL_JSON("select * from articles where title like '%"+req.params.title+"%'", res)   
});

app.get('/getArticles/:credits/:id', async function(req, res){     
    SQL_JSON("select * from articles where credits like '%"+req.params.credits+"%' and id != "+req.params.id, res)   
});

app.get('/getArticle/:number', async function(req, res){     
    SQL_JSON("select * from articles where id = "+req.params.number, res)   
});
app.get('/getComments/:id', async function(req, res){     
    SQL_JSON("select * from comentarios where idComentario = "+req.params.id, res)   
});

app.get('/listPictures', async function(req, res){     
    let arquivos = await listarArquivosDoDiretorio('./public/newImages');
...
app.get('/', async function(req, res){ 
    try{
        createConection()
    conn.query("select *, replace(title, ' ','_') as link from articles where category not like '%destak%' order by createdAt desc", function(error, results, fields){
        if(error){         
            //console.log(error)
            process.exit(1)
...
app.get('/design', async function(req, res){ 
    try{
        createConection()
         conn.query("select *, replace(title, ' ','_') as link from articles where category like '%design%'", function(error, results, fields){
             if(error){         
                 //console.log(error)
                 process.exit(1)
                  res.render('index', {ERROR:error});  
                 ...
                 //console.log(results)        
                  res.render("design",   {ARTICLES:results, categoria:"DESIGN"});    
                 conn.destroy()
             }
         });
...
app.get('/games', async function(req, res){ 
    try{
        createConection()
         conn.query("select *, replace(title, ' ','_') as link from articles where category like '%games%'", function(error, results, fields){
             if(error){         
                 //console.log(error)
                 process.exit(1)
                  res.render('index', {ERROR:error});  
                 ...
                 //console.log(results)        
                  res.render("games",   {ARTICLES:results, categoria:"GAMES"});    
                 conn.destroy()
             }
         });
...
app.get('/news', async function(req, res){ 
   try{
       createConection()
        conn.query("select *, replace(title, ' ','_') as link from articles where category like '%news%'", function(error, results, fields){
            if(error){         
                //console.log(error)
                process.exit(1)
                 res.render('index', {ERROR:error});  
                ...
                //console.log(results)        
                 res.render("news",   {ARTICLES:results, categoria:"NOTÍCIAS"});    
                conn.destroy()
            }
        });
...
app.get('/artigo/:title', async function(req, res){ 
     res.render('artigo', {TITLE:req.params.title.replace(/ /g,"")});
});
...
console.log("Port "+process.env.port_listen)
